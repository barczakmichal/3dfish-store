...
# treefish Store Deployment Script
...
NEON_PROJECT=$(npx neonctl projects create --name "treefish-store" --api-key "$NEON_API_KEY" --output json 2>/dev/null)
...
npx vercel env add NEXTAUTH_URL production --token "$VERCEL_TOKEN" <<< "https://treefish.vercel.app" 2>/dev/null || true
